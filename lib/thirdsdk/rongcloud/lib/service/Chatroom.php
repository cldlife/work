...
class Chatroom {
  ...
  public function create ($chatRoomInfo) {
    try {
      if (empty($chatRoomInfo)) throw new Exception('Paramer "chatRoomInfo" is required');
  ...
  public function join ($userId, $chatroomId) {
    try {
      if (empty($userId)) throw new Exception('Paramer "userId" is required');
      
      if (empty($chatroomId)) throw new Exception('Paramer "chatroomId" is required');
  ...
  public function queryUser ($chatroomId, $count, $order) {
    try {
      ...
      if (empty($count)) throw new Exception('Paramer "count" is required');
      
      if (empty($order)) throw new Exception('Paramer "order" is required');
  ...
  public function addGagUser ($userId, $chatroomId, $minute) {
    try {
      ...
      if (empty($minute)) throw new Exception('Paramer "minute" is required');
