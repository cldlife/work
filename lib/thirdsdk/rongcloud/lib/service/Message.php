...
class Message {
   ...
   * @desc 消息类型&内容模板配置
   * @see name
   * RC:TxtMsg: 文本消息, content 表示文本内容，extra 为附加信息(如果开发者自己需要，可以自己在 App 端进行解析)
   * RC:ImgMsg: 图片消息, content 表示图片缩略图，格式为 JPG，大小不超过 30k，注意在 Base64 进行 Encode 后需要将所有 \r\n 和 \r 和 \n 替换成空，imageUri 为图片 Url
   */
  public $messageTypes = array(
  	array('name' => 'RC:TxtMsg', 'content' => array(
  	  'content' => '',
      'user' => '',
	  'extra' => ''
    )),
    array('name' => 'RC:ImgMsg', 'content' => array(
      'content' => '',
      'user' => '',
      'imageUri' => '',
      'extra' => ''
    )),
    array('name' => 'RC:ImgTextMsg', 'content' => array(
      'title' => '',
      'content' => '',
      'user' => '',
      'imageUri' => '',
   ...
   * @return $json
   */
  public function publishPrivate ($fromUserId, $toUserId, $objectName, $content, $pushContent = '', $pushData = '', $count = 1, $verifyBlacklist = 0, $isPersisted = 1, $isCounted = 1) {
    try {
      if (empty($fromUserId)) throw new Exception('Paramer "fromUserId" is required');
      
      if (empty($toUserId)) throw new Exception('Paramer "toUserId" is required');
      
      if (empty($objectName)) throw new Exception('Paramer "$objectName" is required');
      
      if (empty($content)) throw new Exception('Paramer "$content" is required');
      ...
      if (empty($ret)) throw new Exception('bad request');
      return $ret;
  ...
  public function publishTemplate ($templateMessage) {
    try {
      if (empty($templateMessage)) throw new Exception('Paramer "templateMessage" is required');
  ...
  public function publishGroup ($fromUserId, $toGroupId, $objectName, $content, $pushContent = '', $pushData = '', $isPersisted, $isCounted) {
    try {
      ...
      if (empty($toGroupId)) throw new Exception('Paramer "toGroupId" is required');
  ...
  public function publishDiscussion ($fromUserId, $toDiscussionId, $objectName, $content, $pushContent = '', $pushData = '', $isPersisted, $isCounted) {
    try {
      ...
      if (empty($toDiscussionId)) throw new Exception('Paramer "toDiscussionId" is required');
  ...
  public function publishChatroom ($fromUserId, $toChatroomId, $objectName, $content) {
    try {
      ...
      if (empty($toChatroomId)) throw new Exception('Paramer "toChatroomId" is required');
  ...
  public function getHistory ($date) {
    try {
      if (empty($date)) throw new Exception('Paramer "date" is required');
