...
class SMS {
  ...
  public function getImageCode ($appKey) {
    try {
      if (empty($appKey)) throw new Exception('Paramer "appKey" is required');
  ...
  public function sendCode ($mobile, $templateId, $region, $verifyId = '', $verifyCode = '') {
    try {
      if (empty($mobile)) throw new Exception('Paramer "mobile" is required');
      if (empty($templateId)) throw new Exception('Paramer "templateId" is required');
      if (empty($region)) throw new Exception('Paramer "region" is required');
  ...
  public function verifyCode ($sessionId, $code) {
    try {
      if (empty($sessionId)) throw new Exception('Paramer "sessionId" is required');
      if (empty($code)) throw new Exception('Paramer "code" is required');
