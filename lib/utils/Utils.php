...
class Utils {
	...
	/**
	 * Long类型id的生成实现类：server id（4位十进制数） + 随机数2位十进制数） + 时间戳（遗留13位十进制数）
	 *
	...
	static public function longIdGenerator($serverId = 1) {
		$maxServerId = 9222;
		if (! $serverId)
			$serverId = mt_rand ( 1, $maxServerId );
...
EOT;
	...
	static public function isFromShihuo() {
		...
		// 验证是否来自app
		$isFromShihuo = FALSE;
		if (stripos ( $fromapp, 'ShiHuo' ) !== FALSE)
			$isFromShihuo = TRUE;
		return $isFromShihuo;
	}
	...
	static public function isFromWanZhu() {
		...
		// 验证是否来自app
		$isFromWanZhu = FALSE;
		if (stripos ( $fromapp, 'WanZhu' ) !== FALSE)
			$isFromWanZhu = TRUE;
		return $isFromWanZhu;
	}
	 ...
	 * gzip解压
	 *
	 * @example 页面gzip压缩检测(31139则数据已进行压缩, 并进行解压操作)
	...
	/**
	 * 从list中获取第1个和最后一个 created_time（时间戳）
	 */
	...
	static public function calcDaysBeforeThisMonth($startTime = 0) {
		...
		if ($startTime) {
			...
			if ($days >= $beforeEndDay)
				$days = $days - $beforeEndDay;
		}
		return $days;
	}

	/**
	 * 验证Url是否是本地Url(非http://或https://开头)
	 */
