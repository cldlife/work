...
<div id="content">
  ...
  <div class="itemlist">
    <a class="confirm_btn" id="btn-choose-a"><span>&nbsp;选择文件&nbsp;</span></a>
    <a class="confirm_btn" id="btn-confirm-a" style="display:none;"><span>&nbsp;开始上传&nbsp;</span></a>
    <input type="file" id="attachment" name="attachment" multiple style="visibility: hidden;"/>
    <br/> <br/>
...
if (window.File && window.FileReader && window.FileList && window.Blob && window.btoa) {
    ...
    if (_this.files.length) {
      for (var i=0; i<_this.files.length; i++) {
        ...
        if (file.size > maxSize) {
          ...
          continue;
        } else if (attachmentList.length >= maxLen) {
          break;
        }
      ...
      if (attachmentList.length) {
        ...
        for (var i=0; i<attachmentList.length; i++) {
          html += '<div class="file"><div class="file-info"><span class="file-name" title="'+ attachmentList[i].name +'">'+ attachmentList[i].name +'</span>';
          html += '<span class="file-size">'+ getFileSize(attachmentList[i].size) +'</span></div><div class="progress-container"><div class="progress"></div></div>';
  ...
  function uploadFiles () {
    if (!attachmentList.length) return false;
    for (var i=0; i<attachmentList.length; i++) {
      (function (i) {
        ...
        reader.onload = function (e) {
          ...
          xhr.open("POST", "<?php echo $this->getDeUrl('attachments/upload/file', array('id' => $this->permissionId)); ?>");
          ...
          data.append(attachmentList[i].name, fileStr);
          xhr.send(data);
          reader = null;
        }
