...
class BaseController extends CController {
	...
	public function filters() {
		...
		// session会话验证
		$bkAdminUser = array ();		// 获取当前管理用户
		if ($bkAdminUid)
			$bkAdminUser = $this->getBkAdminService()->getBkAdminUserByUid ($bkAdminUid);
		if ($bkAdminUser && $bkAdminPassword == $bkAdminUser ['passwd'] . self::BKMAN_PASSWORD_KEY ) {
			// 添加用户默认权限
			$bkAdminUser ['permission_ids'] = array_merge ( $bkAdminUser ['permission_ids'], array (
					9999
			) );
			// 是否是最高权限管理员
			...
			if ($loginPage)
				$this->redirect ( $this->getDeUrl ( 'main/index' ) );
		} else {
			if (! $loginPage)
				...
				if ($totalPages >= $middlePageSize) {
					...
					if ($curPage >= $middlePageSize) {
						...
						if ($curPage > ($totalPages - $offset)) {
							if ($i < ($totalPages - $middlePageSize))
								continue;
						} else {
							if ($i <= ($curPage - $offset))
								continue;
					...
					if ($curPage < ($totalPages - $offset)) {
						...
						if ($curPage < $middlePageSize) {
							if ($i > ($middlePageSize + 1))
								continue;
						} else {
							if ($i > ($curPage + $offset - 1))
								continue;
						}
					}
				}
